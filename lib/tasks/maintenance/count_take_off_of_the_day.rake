...
namespace :maintenance do
	...
	task :count_take_off_of_the_day => :environment do
		log = ActiveSupport::Logger.new('log/count_take_off_of_the_day.log')
		start_time = Time.now

		puts "Count Take Off of the Day..."
		log.info "Count Take Off of the Day..."
		
		day_list = Audit.where("created_at >= ?", Time.zone.now.beginning_of_day)
		UnitMailer.send_take_off_of_the_day(day_list).deliver!
