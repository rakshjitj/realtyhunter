...
namespace :maintenance do
	desc "Count Pending Of The Day"
	task :count_pending_of_the_day => :environment do
		log = ActiveSupport::Logger.new('log/count_pending_of_the_day.log')
		start_time = Time.now

		puts "Count Pending of the Day..."
		log.info "Count Pending of the Day..."
		
		day_list = Audit.where("created_at >= ?", Time.zone.now.beginning_of_day)
		UnitMailer.send_pending_of_the_day(day_list).deliver!
