namespace :maintenance do
	...
	task update_status_add_new: :environment do
	    ...
	    for i in 0..page_count_limit-1
	      ...
	      open_house = nil
				...
				if unit
					...
					listing = ResidentialListing.create!({
						unit: new_unit,
						beds: beds,
						baths: item['bathrooms'],
						notes: description,
					...
					item['unit_amenities'].each{ |a| 
						...
						if amenity
							listing.residential_amenities << amenity
							puts "\t residential[id: #{listing.id}] amenity: #{amenity_name}"
							log.info "\t residential[id: #{listing.id}] amenity: #{amenity_name}"
						end
					}

					if item['pets'] && !listing.unit.building.pet_policy
		      	...
		      	pet_policy = PetPolicy.find_or_create_by(name: policy_name, company: company)
						listing.unit.building.pet_policy = pet_policy
					end
