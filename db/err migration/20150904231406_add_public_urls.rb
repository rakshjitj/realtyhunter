class AddPublicUrls < ActiveRecord::Migration
  def change
  	...
  	company = Company.find_by(name: 'MyspaceNYC')
		...
		units.each do |unit|
			unit.update_columns(public_url: "http://myspacenyc.com/listing/MYSPACENYC-#{unit.listing_id}")
		end

		users = User.unarchived.where(company_id: company.id).joins(:company)
		users.each do |user|
	    email_md5 = Digest::MD5.hexdigest(user.email)
			user.update_columns(public_url: "http://myspacenyc.com/agent/AGENT-#{email_md5}")
		end
