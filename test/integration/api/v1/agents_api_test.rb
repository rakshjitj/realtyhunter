...
class AgentsTest < ActionDispatch::IntegrationTest
	...
	test 'valid authentication with token' do 
		get '/v1/agents', {}, {'Authorization' => token_header(@user.auth_token) }
		assert_equal 200, response.status
		assert_equal Mime::JSON, response.content_type
	end

	test 'invalid authentication' do
		get '/v1/agents', {}, {'Authorization' => @user.auth_token + 'fake' }
	...
	test 'returns single agent' do
		user = users(:michael)
		get "/v1/agents/#{user.id}", {}, {'Authorization' => token_header(@user.auth_token) }
		assert response.success?
	...
	test 'returns agents in JSON' do
		get '/v1/agents', {}, {'Accept' => Mime::JSON, 'Authorization' => token_header(@user.auth_token)  }
		assert_equal 200, response.status
		assert_equal Mime::JSON, response.content_type
	end

	test 'requesting agents in XML should return JSON' do
		get '/v1/agents', {}, {'Accept' => Mime::XML, 'Authorization' => token_header(@user.auth_token)  }
		assert_equal 200, response.status
		assert_equal Mime::JSON, response.content_type
	end
